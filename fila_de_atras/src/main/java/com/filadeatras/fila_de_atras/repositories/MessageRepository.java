...
public interface MessageRepository extends JpaRepository<Message, Long>{
	...
	@Query("select msg from Message msg where ((msg.messageAddressee.id=?1 and msg.messageSender.id=?2)or(msg.messageAddressee.id=?2 and msg.messageSender.id=?1)) and msg.messageDeleted=false order by msg.id desc ")
